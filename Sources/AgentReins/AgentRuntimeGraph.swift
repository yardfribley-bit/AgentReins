...
struct AgentRuntimeGraph: Equatable, Sendable {
    ...
    static func build(processes: [ProcessSnapshotRecord], agent: String?) -> AgentRuntimeGraph {
        ...
        if agent?.lowercased().contains("workbuddy") == true {
            ...
            let active = classified.first { ["workbuddy-active-agent", "workbuddy-prewarm-pool"].contains($0.1.componentId) }
            let sandbox = classified.first { $0.1.componentId == "sandbox" }
            if let active, let sandbox, !edges.contains(where: { $0.sourcePID == active.0.pid && $0.targetPID == sandbox.0.pid }) {
                edges.append(RuntimeRelationship(id: "runtime:\(active.0.pid):\(sandbox.0.pid)",
                    sourcePID: active.0.pid, targetPID: sandbox.0.pid,
