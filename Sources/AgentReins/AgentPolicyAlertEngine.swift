...
enum AgentPolicyAlertEngine {
    static func findings(events: [GuardEvent]) -> [GuardEvent] {
        ...
        for event in ordered where event.kind == "file" && event.op == "read" {
            ...
            output.append(alert(rule: sensitive ? "cross_project_sensitive_file_read" : "cross_project_file_read",
                severity: sensitive ? "critical" : "high", event: event, workspace: call.path,
                detail: sensitive
        ...
        for result in ordered where result.kind == "tool" && result.op == "result" {
            ...
            output.append(alert(rule: "credential_in_tool_result", severity: "critical", event: result,
                                workspace: calls[key(result) ?? ""]?.path,
    ...
    private static func alert(rule: String, severity: String, event: GuardEvent,
        ...
        return GuardEvent(id: id, kind: "alert", ruleId: rule, path: event.path,
            ...
            action: "needs_review", sessionId: event.sessionId, traceId: event.traceId,
